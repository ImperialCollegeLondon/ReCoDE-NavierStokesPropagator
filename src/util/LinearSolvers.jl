...
function define_sys_pressure!(
    ...
    # Main Diagonal
    sys.D[2:end-1] =
        -(kx^2 + kz^2) .- (1.0 ./ (domain.dy_F[2:end-1] .* domain.dy[2:end])) -
        (1.0 ./ (domain.dy_F[2:end-1] .* domain.dy[1:end-1]))
...
# Solve System
function solve_sys_pressure_update!(
    sys::TriDiagonalMatrix{Complex{T}},
    sol::Vector{Complex{T}},
    domain::DomainDescriptor,
    kx::T,
...
# Solve System
function solve_sys_pressure_poisson!(
    sys::TriDiagonalMatrix{Complex{T}},
    sol::Vector{Complex{T}},
    R::Vector{Complex{T}},
    kx::T,
